%    example_choi_et_al_percentile_score - Rationality_Measures Copyright (C) 2022  Lasse Mononen
%
...
%
% An example of calculating the percentile score predictive power adjustments 
% for the measures of rationality for the choices in the experiment from 
% Choi et al., 2014, "Who Is (More) Rational?", American Economic Review. 
%
...
prob_strictly_less_rational_than_random=zeros(agents,total_no_measure);    
prob_weakly_less_rational_than_random=zeros(agents,total_no_measure);    
prob_random_satisfies_garp = zeros(agents,1);

% The simulation in predictive_score run in parallel
for i=1:agents
    i
    P=squeeze(agentP(i,:,:));
    Q=squeeze(agentQ(i,:,:));    
    [prob_strictly_less_rational_than_random(i,:),prob_weakly_less_rational_than_random(i,:),prob_random_satisfies_garp(i)]=percentile_score(P, Q, power_vec, sample_size);
end 
...
predictive_score = [header;prob_strictly_less_rational_than_random]
